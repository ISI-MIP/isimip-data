            ...
            if isinstance(category_sector_resource, str):
                ...
                if category_sector_resource_tree.get(category_sector_resource):
                    links.append(
                        (
                            simulation_round,
                            reverse('search', args=[f'tree/{simulation_round}/{product}/{category_sector_resource}']),
                        )
                    )
            else:
                # this is a resource
                for resource_path in category_sector_resource.paths:
                    ...
                    if resource_simulation_round == simulation_round and resource_product == product:
                        tree_publication = category_sector_resource_tree.get(
                            resource_publication
                        ) or category_sector_resource_tree.get(resource_publication.lower())
                        if tree_publication:
                            publication = tree_publication.get('specifier')
                            links.append(
                                (
                                    simulation_round,
                                    reverse('search', args=[f'tree/{simulation_round}/{product}/{publication}']),
                                )
                            )
