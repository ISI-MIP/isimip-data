...
def get_jsonld(request, obj):
    if isinstance(obj, Resource):
        ...
        resources = [get_jsonld(request, resource) for resource in obj.resources.order_by('-doi')]
...
def get_jsonld_datacite(obj):
    return {
        ...
        'keywords': [subject['subject'] for subject in obj.datacite.get('subjects', []) if subject.get('subject')],
        'publisher': {
            '@type': 'Organization',
        ...
        'creator': [get_jsonld_datacite_name(creator) for creator in obj.datacite.get('creators', [])],
        'contributor': [get_jsonld_datacite_name(contributor) for contributor in obj.datacite.get('contributors', [])],
    }
