...
def get_download_operations(request):
    return [
        {
            key: get_template(value).render(request=request) if key == 'template' else value
...
def get_quote_text(head, text, level, add_newline=False):
    ...
    quote = f'{head_indent}{head}\r\n' + '\r\n'.join(
        textwrap.wrap(text, width=70, initial_indent=text_indent, subsequent_indent=text_indent)
    )
